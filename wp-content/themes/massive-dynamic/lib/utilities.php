...
function pixflow_ImportDummyData()
{
    ...
    if($setting){
        ...
        $customizerResponse = ( $wp_filesystem->exists( $d_path.'customizer.dat' ) ) ? $wp_filesystem->get_contents( $d_path.'customizer.dat' ) : '';
        if($customizerResponse == ''){
            echo('customizer.dat does not exist!');
        }
        $contentResponse = ( $wp_filesystem->exists( $d_path.'content.xml' ) ) ? $wp_filesystem->get_contents( $d_path.'content.xml' ) : '';
        if($contentResponse == ''){
            echo('content.xml does not exist!');
        }
    ...
    if($widget){
        ...
        $widget_data->ajax_import_widget_data($upload_dir['basedir'] . '/demo/demo'.$demo.'/widget.json');
    }
...
//return the content
function pixflow_the_content($content){
    remove_filter('the_content','pixflow_the_content');
    if(isset($_SESSION['vc_temp_content'])){
        $content = $_SESSION['vc_temp_content'];
        unset($_SESSION['vc_temp_content']);
    }
    return $content;
}
add_filter( 'the_content', 'pixflow_the_content', 1 );
...
add_action('after_switch_theme', 'pixflow_themeActivation');
function pixflow_themeActivation () {
    if(get_option('show_on_front') == 'posts' || (get_option('show_on_front') == 'page') && !is_object(get_post(get_option('page_on_front')))){
        ...
        if ($my_query->have_posts()) {
            ...
            $page['post_title']   = esc_attr__('Empty Page','massive-dynamic');
            $pageid = wp_insert_post ($page);
            update_option( 'show_on_front', 'page' );
            update_option( 'page_on_front', $pageid );
            update_post_meta( $pageid, 'pixflow_empty_page', 'true' );
        }
        wp_reset_postdata();
    }
}
