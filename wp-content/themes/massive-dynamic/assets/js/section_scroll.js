        ...
        if(type != 'click'){
            ...
            $('.vc_row:not(.vc_inner)').each(function(){
                if(count > parseInt($prev_row.attr('data-index')) ){
                    TweenMax.to($(this), 0, {top: window_height  , scale: 1 , opacity:1 , zIndex: 8 , ease: Power4.easeOut});
                }
                count++;
            });
        }
...
function pixflow_footer_scroll_up_visible(){
    ...
    TweenMax.to( $('footer') , 1 , {bottom:"-" + footer_height +"px" , opacity: 0 , repeatDelay:0.5, ease:Power4.easeOut, onComplete:function(){
        do_scroll = true ;
        footer_show = false;
        pixflow_set_event_on_window(row_count);
    }});
