...
function pixflow_footerPosition() {
    ...
    if($footer.length && !$footer.hasClass('footer-parallax')){
        var $contentHeight = $('body').height() - $( '.layout > .wrap' ).height() + parseInt($('html').css('margin-top'));

        var $PixflowFooterTopSpace = $('div#pixflow_footerTopSpace');

        if( ($('.layout > .wrap').height()-$footer.height()) <= $('body').height() ) {
            $PixflowFooterTopSpace.remove();
            $footer.prepend("<div id='pixflow_footerTopSpace'></div>");
            $('div#pixflow_footerTopSpace').css('height', $contentHeight + 'px');
        }
        else {
            $PixflowFooterTopSpace.css('height', '0');
        }
    }
...
function pixflow_footerParallax(){
    ...
    if($('main').hasClass('has-parallax-footer') && $(window).width() > 1024 && pixflow_isTouchDevice() == false ){
...
function pixflow_modernTabshortcode(tabid) {
    ...
    if (!$('.' + tabid.toString() ).data("ui-tabs")) {
        ...
        $('.'+tabid.toString() + '.ui-tabs-nav li').first().click()
    }
